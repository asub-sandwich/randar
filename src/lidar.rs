...
impl Lidar {
    ...
    pub fn generate(&self) -> Result<()> {
        match self.file_type {
            ...
            FileType::Txt | FileType::Gz => unimplemented!(),
        }
    }
    ...
    fn generate_las(&self) -> Result<()> {
        ...
        // Simple stats used in point generation
        ...
        let hills: Vec<(f64, f64, f64, f64)> = (0..num_hills)
            ...
            .collect();
        let hills = Arc::new(hills);

        for tid in 0..num_cpus {
            let tx = tx.clone();
            let hills = Arc::clone(&hills);
            ...
            thread::spawn(move || {
                ...
                for _ in (0..num_points).filter(|n| n % num_cpus == tid) {
